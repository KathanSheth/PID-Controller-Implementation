...
int main()
{
  ...
  pid_steer.Init(0.15, 0.0002, 2.5);
  pid_speed.Init(0.3, 0.00, 0.02);
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          speed_value = 0.6 - pid_speed.UpdateError_speed(fabs(cte));
          std::cout << "Angle value is: " << angle << std::endl;
          std::cout << "speed value is: " << speed << std::endl;
          
          
          // ******* IS THIS CORRECT???? ******** //
          //steer_value = deg2rad(angle) - 1 * pid.TotalError();
